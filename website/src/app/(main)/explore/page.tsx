...
export default function ExplorePage() {
        ...
        const fetchInitialData = async () => {
            try {
                const [booksRes, usersRes] = await Promise.all([
                    apiClient.get<any>("/api/books", { page: 1, limit: 12 }),
                    apiClient.get<any>("/api/users/suggested")
                ]);
                setTrending(booksRes.books || []);
    ...
    useEffect(() => {
        const timeout = setTimeout(async () => {
            ...
            setLoading(true);
            try {
                // Single call to discovery API — returns both books & users
                const data = await apiClient.get<any>("/api/discovery/search", { q: query.trim() });
                    ...
                    {loading ? (
                        ...
                        <div className="px-4">
                            <h2 className="text-sm font-black uppercase tracking-widest mb-3 mt-4" style={{ color: "var(--text-muted)" }}>
                                {tab === "books" ? "Trending Books" : "Suggested Users"}
                            </h2>
                            {tab === "books" ? (
                                ...
                                <div className="space-y-2 mt-2">
                                    {suggestedUsers.length > 0 ? suggestedUsers.map((u: any, i: number) => (
                                        ...
                                        <div className="text-center py-16">
                                            <p className="text-5xl mb-3">👤</p>
                                            <p className="font-bold" style={{ color: "var(--text-secondary)" }}>No suggested users</p>
                                        </div>
                                    )}
                                </div>
                            )}
